/**
 * MirrorCore API Utilities
 * 
 * This file contains utility functions for interacting with the MirrorCore backend API.
 */

/**
 * Send a message to the MirrorCore backend
 * 
...
export const getSession = async (sessionId) => {
  try {
    const response = await fetch(`/api/mirrorcore/session/${sessionId}`);
...
export const getUserSessions = async (userId) => {
  try {
    const response = await fetch(`/api/mirrorcore/user/${userId}/sessions`);
...
export const setSessionFlag = async (sessionId, flagName, value) => {
  try {
    const response = await fetch(`/api/mirrorcore/flag/${sessionId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
