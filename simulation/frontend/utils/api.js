...
export const getSession = async (sessionId) => {
  try {
    const response = await fetch(`/mirrorcore/session/${sessionId}`);
...
export const getUserSessions = async (userId) => {
  try {
    const response = await fetch(`/mirrorcore/user/${userId}/sessions`);
...
export const setSessionFlag = async (sessionId, flagName, value) => {
  try {
    const response = await fetch(`/mirrorcore/flag/${sessionId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
